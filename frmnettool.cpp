...
void frmNetTool::initForm()
{
	...
	foreach (QHostAddress addr, addrs) {
		...
		if (ip.startsWith("192.168")) {
            this->setWindowTitle(QString("光伏清扫机升级助手V0.1 本机IP[%1] （内部软件，切勿外传）").arg(ip));
			break;
		}
    }
    ...
    transCtrl = new Transctrl();
    connect(transCtrl, SIGNAL(Log(QString)),this, SLOT(appeedLog(QString)));
    sendfileThread = new SendFileThread(this);
    connect(sendfileThread, SIGNAL(Log(QString)),this, SLOT(appeedLog(QString)));
    sendfileThread->start();
    connect(this,    SIGNAL(sendUpdateFile(frmNetTool *)),
            sendfileThread, SLOT(UpdateBin(frmNetTool *)));
    connect(sendfileThread, SIGNAL(updateSendPBar(qint64 ,qint64)),this, SLOT(updateSendPBar(qint64 ,qint64)));


    ///节点表格
    tv = new TableViewDeal(ui->tableViewNode);
    connect(transCtrl, SIGNAL(addNode(int)),tv, SLOT(addNode(int)));
    connect(sendfileThread, SIGNAL(updateProBar(qint64,qint64,qint64)),tv, SLOT(updateProBar(qint64,qint64,qint64)));
    connect(sendfileThread, SIGNAL(updateStatus(qint64,QString)),tv, SLOT(updateStatus(qint64,QString)));
...
void frmNetTool::appeedLog(QString string)
{
    ui->txtDataLog->append(string);
}
...
void frmNetTool::sendDataTcpServer(QByteArray buffer)
{
...
void frmNetTool::on_btnSendTcpServer_clicked()
{

}

void frmNetTool::on_btnSendUpdate_clicked()
{
   // QByteArray data = transCtrl->CalUpdateCommad(2,0);
  //  sendDataTcpServer(data);

    emit sendUpdateFile(this);

}

void frmNetTool::on_btnSelect_clicked()
{
    QString filter = "所有文件 (*.*)";
    QString fileName = myHelper::GetFileName(filter) ;
    ui->txtSendFile->setText(fileName);
    tcpServer->fileName = fileName;
    ui->pbarSendData->setRange(0, 100);
    ui->pbarSendData->setValue(0);


}
void frmNetTool::updateSendPBar(qint64 size,qint64 maxSize)
{
    double value = ( (double)size/(double)maxSize) *100;
    ui->pbarSendData->setValue(value);
    qDebug("maxsize= %d , size = %d",size,maxSize);
}
void frmNetTool::on_txtDataTcpServer_destroyed()
{

}

void frmNetTool::on_pbarSendData_valueChanged(int value)
{
